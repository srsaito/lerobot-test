...
class TensorBoardLogger:
    ...
    def __init__(self, cfg: TrainPipelineConfig):
        ...
        if getattr(self.cfg, "comment", None):
            run_name = f"{run_name}_{self.cfg.comment}"

        self._writer = SummaryWriter(
            log_dir=os.path.join(log_dir, run_name),
            flush_secs=getattr(self.cfg, "flush_secs", 120),
        )

        print(colored("Logs will be synced with TensorBoard.", "blue", attrs=["bold"]))
        logging.info(
            f"Track this run --> "
            + colored(f"tensorboard --logdir={log_dir}", "yellow", attrs=["bold"])
        )

    def log_policy(self, checkpoint_dir: Path):
        """Checkpoint the policy path to TensorBoard as text info."""
        if getattr(self.cfg, "disable_artifact", False):
            return
        step_id = checkpoint_dir.name
        model_path = checkpoint_dir / PRETRAINED_MODEL_DIR
        self._writer.add_text(
            f"checkpoints/{step_id}", f"Model checkpoint saved at: {model_path}",
        )
